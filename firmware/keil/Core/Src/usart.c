...
//重定义fputc函数 
#define UART_TX_WAIT_MAX  20000U

static void uart_putc_timeout(USART_TypeDef *USARTx, u8 ch)
{
	u32 wait = UART_TX_WAIT_MAX;
	while(((USARTx->SR & 0X40) == 0) && wait)
	{
		wait--;
	}
	if(wait)
...
{      
	if(Flag_Show==0)
	{	
	  uart_putc_timeout(USART3, (u8)ch);//Flag_Show=0  使用串口3   
	}
	else
	{	
		uart_putc_timeout(USART1, (u8)ch);//Flag_Show!=0  使用串口1   
	}	
	return ch;
}
#endif
