...
#define APP_UART_TX_WAIT_MAX  20000U

static void APP_UART3_SendByte(u8 data)
{
	u32 wait = APP_UART_TX_WAIT_MAX;
...
static void APP_UART3_SendBuffer(const char *buf, int len)
{
	int i;
	for(i = 0; i < len; i++)
	{
		APP_UART3_SendByte((u8)buf[i]);
	}
}
/**************************************************************************
...
void APP_Show(void)
{
	char frame[192];
	int len;
	len = sprintf(frame,"{%.3f:%.3f:%.3f:%.3f:%.3f:%.3f:%.3f:%.3f:%.3f:%.3f:%.3f:%.3f:%.3f}\r\n",
       theta_1,
       theta_1_dot,
       theta_2,
       theta_2_dot,
	...
	if(len > 0)
	{
		...
		APP_UART3_SendBuffer(frame, len);
	}
