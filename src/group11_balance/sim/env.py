...
class TwoStageBalanceEnv(gym.Env):
    ...
    def __init__(
        ...
        *,
        ...
        high = np.array([40.0, 40.0, 40.0, 40.0, np.pi / 6, 20.0, np.pi / 4, 20.0], dtype=np.float32)
    ...
    def reset(self, *, seed: int | None = None, options: dict | None = None):
        ...
        self.steps = 0
        self.previous_action = np.zeros(2, dtype=np.float32)
        return self._observe(), {"level": self.level}
    ...
    def step(self, action):
        ...
        info = {"physical_action": physical, "failure_reason": reason, "level": self.level}
